...
//            return (Value)(state.get(e));
        ...
        Value applyBinary(Operator op, Value v1, Value v2){
            //Semantics.check(v1.type() == v2.type(), "mismatched types");
            if (op.ArithmeticOp()) {
            ...
            if (op.RelationalOp()) {
                if (v1.type() == Type.FLOAT || v2.type() == Type.FLOAT){
                    ...
                    if(v1.type() == Type.INT){
                        if (op.val.equals(Operator.LT)) 
                            return new BoolValue(v1.intValue( ) < v2.floatValue( ));
                        if (op.val.equals(Operator.LE))
                            return new BoolValue(v1.intValue( ) <= v2.floatValue( ));
                        if (op.val.equals(Operator.EQ))
                            return new BoolValue(v1.intValue( ) == v2.floatValue( ));
                ...
                if (v1.type() == Type.INT || v2.type() == Type.INT){
                    ...
                    if (v1.type() == Type.CHAR){
                        if (op.val.equals(Operator.LT)) {
                            return new BoolValue(v1.charValue( ) < v2.intValue( )); }
                        if (op.val.equals(Operator.LE))
                            return new BoolValue(v1.charValue( ) <= v2.intValue( ));
                        if (op.val.equals(Operator.EQ))
                            return new BoolValue(v1.charValue( ) == v2.intValue( ));
                    ...
                    if (v2.type() == Type.CHAR){
                        if (op.val.equals(Operator.LT)) {
                            return new BoolValue(v1.intValue( ) < v2.charValue( )); }
                        if (op.val.equals(Operator.LE))
                            return new BoolValue(v1.intValue( ) <= v2.charValue( ));
                        if (op.val.equals(Operator.EQ))
                            return new BoolValue(v1.intValue( ) == v2.charValue( ));
        ...
        Value applyUnary (Operator op, Value v) {
            //Semantics.check( ! v.isUndef(),"reference to undef value");
            if (op.val.equals(Operator.NOT))
                return new BoolValue(!v.boolValue());
            if (op.val.equals(Operator.NEG)){
                if (v.type == Type.INT)
